            ...
            <div className="flex items-center gap-3 p-4" style={{ borderBottom: '1px solid #E8DDD0' }}>
              ...
              <div className="relative flex-1 min-w-0">
                <input type="date" value={form.dias_limite} onChange={e => set('dias_limite', e.target.value)}
                  ...
                  style={{ ...inputStyle, WebkitAppearance: 'none', appearance: 'none', minWidth: 0, maxWidth: '100%', color: form.dias_limite ? '#2C2422' : 'transparent' }}
                  onFocus={e => Object.assign(e.target.style, inputFocusStyle)}
                  onBlur={e => Object.assign(e.target.style, { ...inputStyle, WebkitAppearance: 'none', appearance: 'none', minWidth: 0, maxWidth: '100%' })} />
                {!form.dias_limite && (
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: '#9A9A9A', fontSize: '16px', fontFamily: 'Jost, system-ui, sans-serif' }}>
                    Fecha límite
                  </span>
                )}
              </div>
